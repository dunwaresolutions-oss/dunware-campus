...
class SearchView(APIView):
    ...
    def get(self, request):
        ...
        # ── students ──────────────────────────────────────────────
        s_hits = (
            students.filter(multi_word_icontains(
                q, ["first_name", "last_name", "preferred_name", "student_number"]
            ))
        ...
        g_qs = Guardian.objects.filter(
            multi_word_icontains(q, ["first_name", "last_name", "email"])
        )
        if not is_admin:
        ...
        if is_admin:
            u_hits = User.objects.filter(
                ...
                multi_word_icontains(q, ["username", "first_name", "last_name", "email"])
            )[:_PER_TYPE]
        ...
        grp_qs = Group.objects.filter(multi_word_icontains(q, ["name", "stage_label"]))
        if not is_admin:
        ...
        if is_admin:
            a_hits = Application.objects.alive().filter(multi_word_icontains(q, [
